...
class Employee extends MY_Controller
{
    ...
    public function add_employee($id = null)
    {
        ...
        if (!empty($id)) {
            $user_id = base_url(urldecode($id));
            $data['employee_id'] = $user_id;//$this->encryption->decrypt($id);
        } else {
        ...
        $this->outlet_model->_table_name = 'tbl_outlets';
        $outlet_info = $this->outlet_model->get_by(array('status' => '1')) ;
